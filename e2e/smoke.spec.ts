...
test.describe('Study Pets — smoke', () => {
  ...
  test('63. a aba de hoje tem marca, e quem está olhando outro dia tem o caminho de volta', async ({ page }) => {
    const erros = vigiarErros(page);
    await abrirApp(page);
    await page.locator('#tour-skip').click();
    await expect(page.locator('#tour-balloon')).toBeHidden();
  ...
  // A mesma porta do 60, medida por outro ângulo: a seção do ritmo tem que parar no MEIO
  // do `.st-scroll` (um alvo de scroll errado passaria verde com `toBeInViewport`), e a
  // porta do calendário — que não tinha teste — continua indo pro lugar dela.
  test('60b. a porta do ritmo centra a seção no scroll, e a porta do calendário continua indo pro lugar dela', async ({ page }) => {
    const erros = vigiarErros(page);
    await abrirApp(page);
    await page.locator('#tour-skip').click();
    await expect(page.locator('#tour-balloon')).toBeHidden();
  ...
  test('64. modo ao vivo com hardcore: cancelar o consentimento não deixa bloco nenhum no plano', async ({ page }) => {
    ...
    await expect(page.locator('.block-row', { hasText: 'Estudo' })).toHaveCount(0);
    await expect(page.locator('#live-start-btn')).toContainText('Começar');
    await expect(page.locator('#live-tally')).toHaveCount(0);
    await expect(page.locator('#focus-overlay')).toBeHidden();
