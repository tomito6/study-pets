...
test.describe('Study Pets — smoke', () => {
  ...
  test('34. arrastar um evento pela alça muda o horário dele, e o plano se refaz em volta', async ({ page }) => {
    await abrirApp(page);
    await page.getByRole('button', { name: '+ Evento' }).click();
    await page.locator('#ev-name').fill('Aula de Cálculo');
    await page.locator('#ev-start').fill('14:00');
  ...
  test('35. arrastar a refeição pergunta se é só hoje; "só este dia" não mexe nos outros', async ({ page }) => {
    await abrirApp(page);
    await page.locator('#tour-skip').click(); // o balão do tour cobre as abas dos dias
  ...
  test.describe('na Semana', () => {
    ...
    test('36. arrastar um evento na Semana leva ele pra outro dia', async ({ page }) => {
      await abrirApp(page);
      await page.getByRole('button', { name: '+ Evento' }).click();
      await page.locator('#ev-name').fill('Aula de Cálculo');
      await page.locator('#ev-start').fill('14:00');
  ...
  test.describe('no celular', () => {
    ...
    test('37. a alça do evento arrasta com o dedo, sem virar seleção de grupo', async ({ page }) => {
      await abrirApp(page);
      const refeicao = page.locator('.block-row.almoco-row');
      await expect(refeicao).toContainText('13:00–14:00');
      await refeicao.scrollIntoViewIfNeeded();
