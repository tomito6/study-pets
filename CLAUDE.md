...
- `src/application/` — casos de uso e leituras do estado. UI chama isto; isto chama domínio/infra:
  ...
  - `timer.ts` — `tryStartTimer` (valida: só bloco de hoje que ainda não terminou — antes da hora fica em espera; devolve o motivo pra UI mostrar o toast), `startTimer`/`stopTimer`/`closeFocus`, o único `setInterval` do timer (detecta o fim: **no foco** → `checkBlock` + som `sucesso` + emenda no bloco seguinte via `chainedBlockAfter`; **fora do foco** → som do tipo + notificação e o timer some), e o áudio (`playSound`, `toggleMute`, `setVolume`). **Não há `notify()` por segundo** — isso faria o app inteiro re-renderizar e recalcular stats
