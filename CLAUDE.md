...
- `src/features/<feature>/` — UI React por feature:
  ...
  - `groups/` — grupos de estudo: `useGroupSelection.ts` (a máquina de estados da seleção de linhas: botão direito, toque longo, botão "Agrupar"), `GroupPanel.tsx` (novo/editar grupo) e `GroupHeader.tsx` (o cabeçalho na lista). A seleção é `useState` do `PlanTab`; o modal também
...
- **Grupos de estudo**: nome + objetivo sobre um trecho do dia ("Análise II · terminar a lista 3"), criados direto no plano. Três portas de entrada pro mesmo estado de seleção (`useGroupSelection`): arrastar com o **botão direito** no desktop, **segurar o dedo** numa linha no celular, ou o botão **"Agrupar"** ao lado de "+ Evento" (toca no primeiro bloco, depois no último; com mouse, passar por cima mostra o intervalo). O intervalo vira o modal `#group-panel` (nome, objetivo opcional; Enter salva). Grupo é **anotação por horário** — `{id, start, end, name, goal}` em `state.groups[dateKey]` — e **nunca entra no `generateBlocks`**: pertencimento se calcula na render (bloco que cabe inteiro no intervalo), então config/evento mudando não corrompe nada, igual aos checks por horário. Cabeçalho `.group-header` antes do primeiro bloco membro, com `feitos/total` e minutos (só estudo e evento contam; pausa é membro visual, com `.in-group`). Tocar no cabeçalho edita/apaga. Validação em `domain/groups.ts`: fim > início, sem sobrepor outro grupo, ≥ 1 estudo/evento dentro. Dia encerrado é read-only; dia **futuro pode** (planejar é o ponto). Cancelar sessão zera. v1 consciente: sem recorrência, sem "matéria"/cores, sem "cumpri o objetivo?" — ver IDEIAS.md.
